...
- [ ] Confirmar ruta de LAS files con el usuario y crear `data/raw/` con un README que documente origen y conteo de pozos
- [ ] Implementar `scripts/inspect_las.py` que liste por pozo: curvas presentes, unidades, rango de profundidad, % NaN por curva
- [ ] Generar `outputs/eda/las_inventory.csv` con el resumen anterior
- [ ] Filtrar pozos con las 4 curvas objetivo (GR, RT, NPHI, DEN o equivalentes) y gaps <10%
...
- [ ] Generar `scripts/check_den_nphi_relation.py` que produce crossplot DEN vs NPHI por pozo y ajuste lineal
- [ ] Decidir si el dataset es viable: ≥8 pozos válidos y relación DEN-NPHI visible; si no, documentar pivote en `documentation/00_dataset.md`
- [ ] Escribir `Dockerfile` basado en `pytorch/pytorch:2.x-cuda12.x-cudnn-runtime` con ruff/mypy/pytest/lasio/jupyter-less stack
- [ ] Escribir `docker-compose.yml` con `runtime: nvidia`, bind mount al proyecto, working dir `/workspace`
- [ ] Escribir `requirements.txt` con versiones pineadas
- [ ] Verificar acceso a GPU desde el contenedor: `nvidia-smi` y `python -c "import torch; assert torch.cuda.is_available()"`
- [ ] Escribir `documentation/00_environment.md` con instrucciones de build/run del contenedor

### Phase 1 — Pipeline de datos y EDA

- [ ] Implementar `src/data_loader.py`: lectura de LAS con lasio, mapeo de curvas a nombres canónicos, retorno de DataFrame por pozo
- [ ] Implementar `src/preprocessing.py`: limpieza de NaN, transformación logarítmica de RT (decisión registrada en docs), normalización por pozo (min-max vs z-score decidido aquí)
- [ ] Implementar `src/lowo.py`: generador de splits Leave-One-Well-Out
- [ ] Implementar `src/dataset.py`: `WellDataset(torch.utils.data.Dataset)` que toma DataFrames procesados y entrega tensores [GR, RT, NPHI] → DEN
- [ ] Implementar `tests/test_data_loader.py`: carga de un LAS dummy y validación de columnas
- [ ] Implementar `tests/test_preprocessing.py`: cobertura de NaN, log-RT, normalización per-well
- [ ] Implementar `tests/test_lowo.py`: verifica que cada pozo aparece exactamente una vez como test y nunca en el train de su propio fold
- [ ] Implementar `tests/test_dataset.py`: shapes, dtypes, longitud
- [ ] Implementar `scripts/run_eda.py`: distribuciones por curva, crossplots, calidad por pozo; figuras a `outputs/eda/`
...
- [ ] Implementar `src/model.py`: clase `MLP` parametrizable (hidden_dims, dropout opcional), defaults 3→64→64→32→1 ReLU
...
- [ ] Implementar `scripts/train_baseline.py`: ejecuta LOWO completo, guarda métricas por fold en `outputs/baseline/metrics.json` y predicciones por pozo en `outputs/baseline/predictions/{well_id}.parquet`
...
- [ ] Implementar `scripts/train_pinn.py`: una corrida LOWO con un λ específico, guarda a `outputs/pinn/lambda_{λ}/`
- [ ] Implementar `scripts/sweep_lambda.py`: ejecuta sweep sobre λ ∈ {0.0, 0.01, 0.05, 0.1, 0.5, 1.0}, agrega métricas a `outputs/pinn/lambda_sweep.json`
...
- [ ] Implementar `scripts/compare_baseline_vs_pinn.py`: comparación pareada por pozo (Δ MAE, signo, significancia), salida a `outputs/figures/comparison_table.csv`
- [ ] Implementar `scripts/plot_results.py`: curvas DEN real vs base vs PINN en profundidad, crossplot con línea física, λ vs error agregado; figuras a `outputs/figures/`
...
- [ ] Verificar reproducibilidad: clonar en directorio limpio, `docker compose up`, ejecutar `scripts/train_baseline.py` y `scripts/sweep_lambda.py` sin intervención
...
docker compose run --rm dev python scripts/train_baseline.py
docker compose run --rm dev python scripts/sweep_lambda.py
docker compose run --rm dev python scripts/compare_baseline_vs_pinn.py
docker compose run --rm dev python scripts/plot_results.py
```
